...
/**
 * InstructorLayout - Layout cho trang giảng viên
...
const InstructorLayout = () => {
  ...
  const menuItems = [
    ...
    { icon: User, label: 'Hồ sơ', href: '/instructor/profile' },
    { icon: BookOpen, label: 'Khóa học', href: '/instructor/courses' },
    { icon: Megaphone, label: 'Thông báo', href: '/instructor/announcements' },
    { icon: FileText, label: 'Bài tập', href: '/instructor/assignments' },
    { icon: Users, label: 'Học viên', href: '/instructor/students' },
    { icon: HelpCircle, label: 'Quiz', href: '/instructor/quiz' },
    { icon: FileCheck, label: 'Kết quả Quiz', href: '/instructor/quiz-results' },
    { icon: Award, label: 'Chứng chỉ', href: '/instructor/certificates' },
  ]

  const earningsItems = [
    { icon: DollarSign, label: 'Thu nhập', href: '/instructor/earnings' },
    { icon: CreditCard, label: 'Rút tiền', href: '/instructor/payout' },
    { icon: FileCheck, label: 'Sao kê', href: '/instructor/statements' },
  ]

  const supportItems = [
    { icon: MessageSquare, label: 'Tin nhắn', href: '/instructor/messages' },
    { icon: HelpCircle, label: 'Hỗ trợ', href: '/instructor/support' },
  ]
        ...
        <div className="h-full w-64 bg-slate-800 border-r border-slate-700 flex flex-col">
          ...
          <div className="h-16 flex items-center justify-between px-4 border-b border-slate-700">
            <Link to="/" className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-xs text-violet-400 block -mt-1">Instructor</span>
              </div>
            </Link>
            <button 
          ...
          <div className="p-4 border-t border-slate-700">
            <div className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg">
              <img
                src={user?.avatarUrl || `https://ui-avatars.com/api/?name=${user?.firstName || 'Instructor'}`}
                alt="Avatar"
                className="w-10 h-10 rounded-full"
              />
              <div className="flex-1 min-w-0">
                <div className="text-white font-medium truncate">
                  {user?.firstName || 'Giảng viên'}
                </div>
                <div className="text-slate-400 text-sm truncate">Instructor</div>
              </div>
            </div>
          </div>
        </div>
